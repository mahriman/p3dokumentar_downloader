...
foreach($podcasts_to_download as $item){
    ...
    if(!is_dir($download_path."/P3 Dokumentär ".$download_year))
    {
      echo "[info] Directory '" . $download_path."/P3 Dokumentär ".$download_year . "' does not exist, creating...\n";
      exec("mkdir -p \"" . $download_path."/P3 Dokumentär ".$download_year . "\"", $output, $exit_status); 
      if($exit_status !== 0)
      {
        echo "[!] Unable to create directory '" . $download_path."/P3 Dokumentär ".$download_year . "', exiting (3)!\n";
        exit(3);
      }
    }

    if(file_exists($download_path."/P3 Dokumentär ".$download_year."/".$download_filename ))
    {
        $continue = false;
        echo "[info] '" . $download_path."/P3 Dokumentär ".$download_year."/".$download_filename . "' already downloaded, skipping... \n";
    }
    
    if($continue == true)
    {
        echo "[+] Downloading '". $download_filename . "' to " . $download_path."/P3 Dokumentär ".$download_year . "...\n";
        exec("wget --quiet --no-check-certificate -O \"" . $download_path."/P3 Dokumentär ".$download_year . "/" . $download_filename . "\"" . " " . $item['link'], $download_output, $download_exit_status);
